<div class="row ppgbo_row ppgbo_product_3">
    <?php
    ...
    <div class="product col-<?php echo (($count == 1 || $counter_line == 1) ? '12' : (($count == 2 || $counter_line == 2) ? '6' : (($counter_line == 3) ? '4' : (($counter_line == 4) ? '3' : '2')))) ?>">
        <?php
            ...
            if ($obfuscation) {
                ...
                echo '<a class="product_link" ' . (($new_tab) ? 'target="_blank"' : '') . ' href="' . esc_url( $product->url_rewrite ) . '">' . esc_html( $product->product->name ) . '</a>';
            }
